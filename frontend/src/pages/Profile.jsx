        ...
        <form className="card space-y-3" onSubmit={submit}>
          <div className="flex items-center gap-4">
            <div className="w-20 h-20 rounded-full overflow-hidden bg-slate-100 flex items-center justify-center">
              {profile.avatarURL ? <img src={profile.avatarURL} alt="avatar" className="w-full h-full object-cover" /> : <div className="text-xl muted">{profile.name?.split(' ').map(n=>n[0]).join('')}</div>}
            </div>
            <div>
