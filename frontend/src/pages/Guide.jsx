      ...
      <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm p-3 sm:p-4 md:p-6 border border-slate-100">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
          <div className="flex-1 min-w-0">
            <h1 className="text-xl sm:text-2xl font-semibold text-slate-900 mb-1">Employee Guide</h1>
            <div className="text-xs sm:text-sm text-slate-500">Step-by-step onboarding instructions and tips</div>
          </div>
          <div className="flex-shrink-0">
            <Link 
              to="/dashboard" 
              className="inline-flex items-center justify-center w-full sm:w-auto px-4 py-2.5 sm:py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 active:bg-slate-100 transition-colors touch-manipulation min-h-[44px] sm:min-h-0"
            >
              Back to Dashboard
            </Link>
          </div>
        ...
        <div className="mt-4 sm:mt-6 space-y-2.5 sm:space-y-3 md:space-y-4">
          {steps.map((s, idx) => (
            <div key={idx} className="border-2 border-slate-200 rounded-lg overflow-hidden">
              <button 
                className="w-full text-left px-3 sm:px-4 py-3 sm:py-3.5 md:py-4 flex items-center justify-between gap-3 touch-manipulation min-h-[44px] active:bg-slate-50 transition-colors" 
                onClick={() => setOpen(open === idx ? -1 : idx)}
              >
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-sm sm:text-base text-slate-900 mb-0.5">{s.title}</div>
                  <div className="text-xs text-slate-500">Step {idx + 1} of {steps.length}</div>
                </div>
                <div className="text-xl sm:text-2xl text-slate-500 font-light flex-shrink-0 w-6 sm:w-8 flex items-center justify-center">{open === idx ? '−' : '+'}</div>
              </button>
              {open === idx && (
                <div className="px-3 sm:px-4 py-3 sm:py-3.5 md:py-4 bg-slate-50 text-xs sm:text-sm text-slate-700 leading-relaxed border-t border-slate-200">
                  {s.desc}
                </div>
              )}
            </div>
        ...
        <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-slate-200">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div className="text-xs sm:text-sm text-slate-500 flex-1 min-w-0">
              <span className="font-medium text-slate-700">Tip:</span> Complete your documents early to speed up onboarding.
            </div>
